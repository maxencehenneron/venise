...
import (
	...
	"github.com/dernise/venise/decoder/OSMPBF"
	"github.com/dernise/venise/structures"
	"github.com/golang/protobuf/proto"
)

// Pbf for Blob with OSMData (PrimitiveBlock)
...
func (dec *dataDecoder) parseNodes(pb *OSMPBF.PrimitiveBlock, nodes []*OSMPBF.Node) {
	...
	for _, node := range nodes {
		...
		dec.q = append(dec.q, &structures.Node{id, latitude, longitude, tags, info, nil})
...
func extractMembers(stringTable []string, rel *OSMPBF.Relation) []structures.Member {
	memIDs := rel.GetMemids()
	types := rel.GetTypes()
	roleIDs := rel.GetRolesSid()

	var memID int64
	members := make([]structures.Member, len(memIDs))
	for index := range memIDs {
		memID = memIDs[index] + memID // delta encoding

		var memType structures.MemberType
		switch types[index] {
		case OSMPBF.Relation_NODE:
			memType = structures.NodeType
		case OSMPBF.Relation_WAY:
			memType = structures.WayType
		case OSMPBF.Relation_RELATION:
			memType = structures.RelationType
		}

		role := stringTable[roleIDs[index]]

		members[index] = structures.Member{memID, memType, role, nil, nil}
	}
...
func (dec *dataDecoder) parseRelations(pb *OSMPBF.PrimitiveBlock, relations []*OSMPBF.Relation) {
	...
	for _, rel := range relations {
		...
		dec.q = append(dec.q, &structures.Relation{id, tags, members, info, nil})
	}
}

func extractInfo(stringTable []string, i *OSMPBF.Info, dateGranularity int64) structures.Info {
	info := structures.Info{Visible: true}
...
func extractDenseInfo(stringTable []string, state *denseInfoState, di *OSMPBF.DenseInfo, index int, dateGranularity int64) structures.Info {
	info := structures.Info{Visible: true}
